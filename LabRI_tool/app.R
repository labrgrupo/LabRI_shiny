...
# C.2. Main Container for the Application Header
  ...
  tags$div(
    ...
    tags$div(
      ...
      h1("Laboratory Reference Interval (LabRI) (v5.0.0)",
    ...
    ## C.4.1 Configuration Tab Panel
      ...
      ### C.4.1.1. Sidebar Panel for Configuration Inputs
         ...
         #### C.4.1.1.2. Input: Define the Dataset
                          ...
                          style = "font-size: 12px; color: #666; 
                          text-align: justify; margin-top: -10px;margin-bottom: 25px;"),
...
# D.4. Reactive Function to Load the File and Read Columns
  ...
  dataset <- reactive({
    ...
    if (ext == "csv") {
      ...
      # tenta comma primeiro (padrao), depois ;, depois tab
      df <- tryCatch(
        read.csv(file_path, sep = ",", dec = ".", header = TRUE,
      ...
      if (is.null(df) || ncol(df) == 1) {
        df <- read.delim(file_path, sep = "\t", dec = ".", header = TRUE,
                         stringsAsFactors = FALSE, check.names = FALSE,
                         fileEncoding = "UTF-8-BOM")
      }
...
# D.8. Observer to Generate Report (with progress tracking)
  ...
  # D.8.0. Reactive values for progress tracking
  rv <- reactiveValues(
    rendering = FALSE,
    bg_process = NULL,
    progress_file = NULL,
  ...
  # D.8.0.1. Chunk labels for progress display
  ...
  observeEvent(input$generate_report, {
    ...
    ## D.8.2. Setup progress tracking file
    ...
    ## D.8.4. Launch background R process for rendering
    ...
    rv$bg_process <- callr::r_bg(
      function(rmd_path, output_file, rmd_params, progress_file, 
        ...
        # Write progress for each knitr chunk
        ...
        # Setup knitr hooks to track chunk execution
  ...
  # D.8.5. Poll progress file and update modal
  observe({
    req(rv$timer_active)
    ...
    # Send UI updates via JavaScript
    js_code <- sprintf("
