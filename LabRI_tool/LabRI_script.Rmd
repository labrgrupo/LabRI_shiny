---
title: "**Report LabRI Tool (v`r params$version`)**"
...
##### 1.3.2.1. Auto-detect delimiter and decimal mark robustly
        ...
        # Decide decimal mark using up to 39 data lines (lines 2 to 40)
        if (delim_detected == ",") {
          decimal_mark <- "."
        } else {
...
#### 1.3.5. Clean and standardize the specified column name ####################
  ...
  # Apply the SAME transformations applied to the data column names,
  # so the comparison works for any column name (with/without accents,
...
#### 1.3.6. Check if the specified column exists in the dataset ################
  ...
  if (!column_name_clean %in% colnames(data)) {
    # Fallback: case-insensitive match (handles dropdown vs. data inconsistencies)
    match_idx <- which(tolower(colnames(data)) == tolower(column_name_clean))
    if (length(match_idx) == 1) {
      column_name_clean <- colnames(data)[match_idx]
    } else {
      stop(paste0("The specified column '", column_name,
...
if (length(resultados) == 0) {
  stop(paste0("No valid numeric values found in column '", Column_Name,
...
#### 1.6.5. Maximum observed n_dec (weighted by Freq > 0, excluding valor_excluir)
...
##### 1.6.5.1. Only consider rows that actually exist in the dataset (Freq > 0)
  
  
  Freq_safe <- ifelse(is.na(Freq_num), 0, Freq_num)
  linha_valida <- !excluir & !is.na(n_dec) & Freq_safe > 0
  
  max_dec_observado <- if (any(linha_valida)) {
    max(n_dec[linha_valida], na.rm = TRUE)
  } else {
    NA_integer_
  }
  
  
#### 1.6.6. Analysis of n_casas_decimais column - preponderant value (WEIGHTED BY Freq)
  
  
##### 1.6.6.1. Select valid rows: non-NA n_dec AND Freq > 0
  
  
  idx_validos <- !is.na(n_dec) & Freq_safe > 0
  n_dec_validos    <- n_dec[idx_validos]
  freq_validos     <- Freq_safe[idx_validos]
  
  if (length(n_dec_validos) > 0) {
    
    
##### 1.6.6.2. Weighted absolute frequency: sum of Freq for each n_dec level
    
    
    freq_ponderada <- tapply(freq_validos, n_dec_validos, sum, na.rm = TRUE)
...
##### 1.6.6.3. Create data frame for auditing
    ...
    auditoria_decimais <- data.frame(
      n_casas_decimais = as.integer(names(freq_ponderada)),
      freq_absoluta    = as.integer(freq_ponderada),
      stringsAsFactors = FALSE
    )
    
    
##### 1.6.6.4. Sort by number of decimal places
    
    
    auditoria_decimais <- auditoria_decimais[order(auditoria_decimais$n_casas_decimais), ]
    
    
##### 1.6.6.5. Add relative frequency (also weighted)
...
##### 1.6.6.6. Identify the predominant value (highest WEIGHTED absolute frequency)
    
    
    idx_preponderante     <- which.max(auditoria_decimais$freq_absoluta)
    max_dec_preponderante <- auditoria_decimais$n_casas_decimais[idx_preponderante]
    ...
    max_dec_preponderante <- NA_integer_
  }
...
#### 1.6.8. Return structured output ###########################################
  
  
  list(
    df                       = tabela.contagem,
    max_decimal              = max_dec_observado,        
    max_decimal_freq         = max_dec_preponderante,    
...
unlink("3_Outputs/Figures/*", recursive = TRUE, force = TRUE)
...
###### 3.1.1.5.1. Initialize trace log ######################################### 
  ...
  .log_msg <- function(level, msg) {
    ...
    if (isTRUE(verbose)) {
      tag <- toupper(substring(level, 1, nchar(level) - 1))  # "warning" -> "WARNING"
...
write_xlsx(data.frame(resultados),
           "3_Outputs/Spreadsheets/1_dataset_subsampled_jittered.xlsx")
...
jpeg(filename = "3_Outputs/Figures/Fig.1.jpeg", 
...
jpeg(filename = "3_Outputs/Figures/Fig.2.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.3.jpeg", 
...
jpeg(filename = "3_Outputs/Figures/Fig.4and5.jpeg",
...
jpeg(filename = "3_Outputs/Figures/Fig.4.jpeg",
     width = 6 * 600, height = 4 * 600, units = "px", 
     res = 600, quality = 100)

Fig.4.refineR.new<-plot(refineR.LabRI.1,
                        Scale = "original",
                        RIperc = RIperc2,
                        showPathol = TRUE,
                        scalePathol = TRUE,
...
titulo.Fig.5 <- bquote(
  atop(
    ...
    jpeg(filename = "3_Outputs/Figures/Fig.5.jpeg",
     width = 6 * 600, height = 4 * 600, units = "px",res = 600, quality = 100)
...
write_xlsx(data.frame(dataset_orig_truncado),
           "3_Outputs/Spreadsheets/2_dataset_subsampled_jittered_truncated.xlsx")
...
jpeg(filename = "3_Outputs/Figures/Fig.6.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.7.jpeg", 
     width = 6 * 600, height = 4 * 600, units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.8.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.9and10.jpeg",
     width = 6 * 600, height = 4 * 600, units = "px", 
     res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.9.jpeg", 
     width = 6 * 600, height = 4 * 600, units = "px", 
     res = 600, quality = 100)

Fig.9.refineR.new<-plot(refineR.LabRI.2,
                        Scale = "original",
                        RIperc = RIperc2,
                        showPathol = TRUE,
                        scalePathol = TRUE,
...
titulo.Fig.10 <- bquote(
  atop(
    ...
    jpeg(filename = "3_Outputs/Figures/Fig.10.jpeg",
     width = 6 * 600, height = 4 * 600, units = "px",res = 600, quality = 100)
...
lambda.method<-paste("\u03BB<sub>LabRI</sub>= ",lambda.LabRI,"; Method= loglik",sep = "")
...
jpeg(filename = "3_Outputs/Figures/Fig.11.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
titulo.fig.12<-paste("Fig. 12.  Ogive graph after outlier exclusion (\u03BB<sub>LabRI</sub> = 1; n<sub>New1</sub> =",n.after.truncation,").")

titulo.fig.12.tranf<-paste("Fig. 12. Ogive graph after transformation (",
                            lambda.method,"; n<sub>New1</sub> =",n.after.truncation,")")
...
jpeg(filename = "3_Outputs/Figures/Fig.12.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
titulo.fig.13<-paste("Fig. 13. Q-Q plot after outlier exclusion (\u03BB<sub>LabRI</sub> = 1; n<sub>New1</sub> =",n.after.truncation,").")
...
jpeg(filename = "3_Outputs/Figures/Fig.13.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
##### Marginal BIC gains are rectified to their positive part, so that the
##### post-peak decline of the BIC profile is not misread as additional gain
##### when accumulating evidence for the Pareto and plateau criteria.


dif.1.para.2 <- pmax(componente[9]-componente[8], 0)
dif.2.para.3 <- pmax(componente[10]-componente[9], 0)
dif.3.para.4 <- pmax(componente[11]-componente[10], 0)
dif.4.para.5 <- pmax(componente[12]-componente[11], 0)
dif.5.para.6 <- pmax(componente[13]-componente[12], 0)
dif.6.para.7 <- pmax(componente[14]-componente[13], 0)
...
##### 3.2.3.6. Criterion 2 - Plateau on the BIC chart (Model V) ################
##### Standardized with Model E: the plateau threshold is taken as the median
##### of four mode estimators of the accumulated-gain vector. After the
##### positive-part rectification above, an early BIC peak yields a constant
##### accumulated-gain vector (zero range), which the kernel-based estimators
##### cannot process. A degeneracy guard returns the attained value directly in
##### that case, and the estimators are otherwise wrapped so any failure falls
##### back to the median of the vector.
...
if (length(unique(vetor.k.2)) <= 1) {

  k.2.moda <- as.numeric(vetor.k.2[1])

} else {

  k.2.moda <- tryCatch({
    k.2.moda.1 <- mlv1(vetor.k.2, method = "Parzen")[1]
    k.2.moda.2 <- mlv1(vetor.k.2, method = "naive")[1]
    k.2.moda.3 <- mlv1(vetor.k.2, method = "Venter")[1]
    k.2.moda.4 <- locmodes(vetor.k.2, mod0 = 1)$locations
    round(median(c(k.2.moda.1, k.2.moda.2, k.2.moda.3, k.2.moda.4)), digits = 2)
  }, error = function(e) {
    round(median(vetor.k.2), digits = 2)
  })
...
dif.11.para.22 <- pmax(componente[2]-componente[1], 0)
dif.22.para.33 <- pmax(componente[3]-componente[2], 0)
dif.33.para.44 <- pmax(componente[4]-componente[3], 0)
dif.44.para.55 <- pmax(componente[5]-componente[4], 0)
dif.55.para.66 <- pmax(componente[6]-componente[5], 0)
dif.66.para.77 <- pmax(componente[7]-componente[6], 0)
...
##### 3.2.4.6. Criterion 2 - Plateau on the BIC chart (Model E) ################
##### Standardized with Model V: the plateau threshold is taken as the median
##### of four mode estimators of the accumulated-gain vector, with explicit
##### two-decimal rounding so the criterion does not depend on an external
##### precision setting. The same degeneracy guard and error fallback apply: a
##### constant accumulated-gain vector returns its attained value directly, and
##### any estimator failure falls back to the median of the vector.
...
if (length(unique(vetor.k.22)) <= 1) {

  k.22.moda <- as.numeric(vetor.k.22[1])
  ...
  k.22.moda <- tryCatch({
    ...
    k.22.moda.4 <- locmodes(vetor.k.22, mod0 = 1)$locations
    round(median(c(k.22.moda.1, k.22.moda.2, k.22.moda.3, k.22.moda.4)), digits = 2)
  }, error = function(e) {
    round(median(vetor.k.22), digits = 2)
  })
...
##### Terminal consensus rule of the cascade. Because the eight inputs are
##### validated as finite, the mode always resolves to a single value; no
##### further fallback is reachable from here.
 
          crit_conclusion <- mode_min_tie(v_all)
...
#### 3.2.5.8. Post-cascade adjustments #########################################

##### 3.2.5.8.1. Invariant guard ##############################################
##### Defensive only. Under validated inputs the cascade always yields a finite
##### value; this guarantees a finite conclusion before the high-k gate and the
##### final numerical guard. The fallback is the mode of the eight criteria.
  
  if (!is.finite(crit_conclusion)) {
    crit_conclusion <- mode_min_tie(v_all)
    rule_used <- paste0(rule_used, "_invariant_mode_guard")
  }

##### 3.2.5.8.2. High-k validation via marginal delta-BIC #####################
##### When the provisional k falls in {5, 6, 7}, the k-th Gaussian component is
##### retained only if its inclusion yields a strong improvement in model
##### evidence, defined as a marginal BIC increment exceeding delta_bic_threshold
##### (default 6, the strong-evidence cut-off on the Kass-Raftery scale; set to
##### 10 to require very strong evidence). The increment is evaluated as the more
##### favorable of the E-model and V-model BIC profiles. If the increment is not
##### strong, the provisional k is reduced by one and the same test is applied to
##### the new marginal component, descending until a strongly supported component
##### is reached. This enforces an asymmetric burden of proof: lowering k is
##### inexpensive, whereas retaining a high k requires strong, model-based
##### corroboration rather than deference to a single nonparametric signal. The
##### gate also neutralizes spurious high-k inputs arising from a degenerate or
##### all-missing BIC profile, since a non-finite increment is treated as no
##### evidence and triggers descent.

  if (crit_conclusion %in% c(5, 6, 7) &&
      !is.null(bic_E) && !is.null(bic_V)) {

    bic_E_vec <- suppressWarnings(as.numeric(bic_E))
    bic_V_vec <- suppressWarnings(as.numeric(bic_V))

    if (length(bic_E_vec) >= 7 && length(bic_V_vec) >= 7) {

      k_validated <- crit_conclusion

      while (k_validated >= 2) {

        delta_E <- bic_E_vec[k_validated] - bic_E_vec[k_validated - 1]
        delta_V <- bic_V_vec[k_validated] - bic_V_vec[k_validated - 1]

        if (!is.finite(delta_E)) delta_E <- -Inf
        if (!is.finite(delta_V)) delta_V <- -Inf

        delta_marginal <- max(delta_E, delta_V)

        if (delta_marginal > delta_bic_threshold) break

        k_validated <- k_validated - 1
      }

      if (k_validated != crit_conclusion) {

        crit_conclusion_original <- crit_conclusion
        crit_conclusion <- k_validated

        rule_used <- paste0(
          rule_used,
          "_then_high_k_reduced_by_delta_bic_gate",
          "_original_k_",
...
k.new <- select_k_9criteria(
  ...
  bic_E = lista.mistura.E,
  bic_V = lista.mistura.V,
  delta_bic_threshold = 6)$crit_conclusion
...
jpeg(filename = "3_Outputs/Figures/Fig.14.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.15.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
##### 3.2.18.3. OPTIMIZATION: reuse fit.1 for JPEG version too #################


Fig.16.new <- plot.hist.mix(fit.1.his, family = "normal")

jpeg(filename = "3_Outputs/Figures/Fig.16.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.17.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.18.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.19.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_Outputs/Figures/Fig.20.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
if (isTRUE(k.new > 1)) {
  ...
  if (!is.null(Fig.21.new)) {

    jpeg(
      filename = "3_Outputs/Figures/Fig.21.jpeg", 
      width = 6 * 600,
      height = 4 * 600, 
      units = "px",
      res = 600,
...
write_xlsx(lambdas.dataset,
           "3_Outputs/Spreadsheets/3_lambdas_dataset.xlsx")
```
...
Moda<-multimode::locmodes(dataset_orig_truncado$dataset_orig_truncado,mod0=1)$locations
Mediana<-median(dataset_orig_truncado$dataset_orig_truncado)

Midpoint_IR<-round(ifelse(Media>Mediana & Media>Moda & 
             Mediana>Moda,Moda,ifelse(Moda>Mediana & 
             Moda>Media & Mediana>Media,Moda,Mediana)),digits = casa.decimal2)
...
##### 3.3.4.2. Validations ####################################################
  ...
  if (lado == "Two_sided" && URL <= LRL) stop("URL must be greater than LRL")
  if (lado == "Right_sided" && URL <= 0) stop("URL must be > 0 for one-sided upper")
  if (lado == "one_sided_lower" && LRL <= 0) {
    stop("LRL must be > 0 for one-sided lower (Box-Cox requires positive values)")
  }
  if (n < 40) stop("The sample size should be at least 40")
  if (any(conf.level <= 0 | conf.level >= 1)) {
    stop("conf.level must be between 0 and 1 (e.g., 0.90, 0.95, 0.99)")
  }
  if (midpoint <= 0) stop("midpoint must be > 0 (Box-Cox requires positive values)")
...
##### 3.3.4.3. Define percentile probabilities and recover mu/sigma ###########
  ...
  if (sigma.t <= 0) stop("Recovered sigma <= 0. Check input values.")

  n.sim <- ifelse(n < 1000, 15000,
                  ifelse(n > 50000,
                         1000, 10000))
...
###### 3.3.4.4.2.1. Generation + inverse Box-Cox transformation in the same block
    ...
    chunk <- matrix(rnorm(b * n, mean = mu.t, sd = sigma.t),
                    nrow = b, ncol = n)
...
###### 3.3.4.4.2.2. Quantiles per row
    ...
    if (!is.null(sim.LRL)) {
      sim.LRL[i.start:i.end] <- matrixStats::rowQuantiles(
        chunk, probs = probs.LRL, useNames = FALSE
      )
    }
    if (!is.null(sim.URL)) {
      sim.URL[i.start:i.end] <- matrixStats::rowQuantiles(
        chunk, probs = probs.URL, useNames = FALSE
      )
    }
...
###### 3.3.4.4.4. STEP 4 - Plot function (ggplot2 + patchwork) ################
  ...
  plot_fn <- function(Title_override = NULL) {
    ...
    plot.main <- if (!is.null(Title_override)) Title_override else
                 if (!is.null(main))           main          else Title

    plot.sub <- paste("Midpoint =", midpoint,
...
###### 3.3.4.5.1. PANEL 1: Histograma de combined
    ...
    p1 <- ggplot(df_comb, aes(x = x)) +
      geom_histogram(aes(y = after_stat(count)),
                     bins = 80, fill = "gray80", color = "gray60",
                     linewidth = 0.2) +
      coord_cartesian(xlim = c(plot.lo, plot.hi)) +
      scale_x_continuous(breaks = waiver(), n.breaks = 10, expand = waiver()) +
      labs(title = plot.main, subtitle = plot.sub,
           x = xlab, y = "Absolute Frequency") +
      theme_mc +
      theme(
    ...
    p1 <- p1 + geom_vline(xintercept = midpoint, color = "black",
                          linewidth = 0.8, linetype = "twodash")

    if (!is.null(sim.LRL)) {
      p1 <- p1 +
        annotate("rect",
                 xmin = row_ci$lower.lim.low, xmax = row_ci$lower.lim.upp,
                 ymin = -Inf, ymax = Inf,
                 fill = col.lower.main, alpha = 0.12) +
        geom_vline(xintercept = LRL, color = col.lower.main,
                   linewidth = 1.2, linetype = "solid") +
        geom_vline(xintercept = row_ci$lower.lim.low, color = col.lower.light,
                   linewidth = 0.7, linetype = "dashed") +
        geom_vline(xintercept = row_ci$lower.lim.upp, color = col.lower.light,
                   linewidth = 0.7, linetype = "dashed")
    }
    ...
    if (!is.null(sim.URL)) {
      p1 <- p1 +
        annotate("rect",
                 xmin = row_ci$upper.lim.low, xmax = row_ci$upper.lim.upp,
                 ymin = -Inf, ymax = Inf,
                 fill = col.upper.main, alpha = 0.12) +
        geom_vline(xintercept = URL, color = col.upper.main,
                   linewidth = 1.2, linetype = "solid") +
        geom_vline(xintercept = row_ci$upper.lim.low, color = col.upper.light,
                   linewidth = 0.7, linetype = "dashed") +
        geom_vline(xintercept = row_ci$upper.lim.upp, color = col.upper.light,
                   linewidth = 0.7, linetype = "dashed")
    }

###### 3.3.4.5.2. PANEL 2: Distribuição LRL
    ...
    if (!is.null(sim.LRL)) {
      ...
      p2 <- ggplot(df_lrl, aes(x = x)) +
        ...
        geom_vline(xintercept = med_lrl, color = col.lower.main,
        ...
        geom_vline(xintercept = row_ci$lower.lim.upp, color = col.lower.light,
                   linewidth = 0.9, linetype = "dashed") +
        scale_x_continuous(breaks = waiver(), n.breaks = 8, expand = waiver()) +
        labs(title = paste(sprintf("LRL = %.2f", LRL),
                           "|", n.sim, "Bootstrap replications"),
             subtitle = sprintf("CI %d%%: %.4f to %.4f",
                                cl_use * 100,
                                row_ci$lower.lim.low, row_ci$lower.lim.upp),
             x = xlab, y = "Absolute Frequency") +
        theme_mc +
        theme(
      ...
      p2 <- p2 +
        geom_line(data = data.frame(x = NA_real_, y = NA_real_, g = sprintf("LRL LabRI = %.2f", LRL)),
                  aes(x = x, y = y, color = g, linetype = g), linewidth = 1.3, na.rm = TRUE) +
        geom_line(data = data.frame(x = NA_real_, y = NA_real_, g = sprintf("Median = %.2f", med_lrl)),
                  aes(x = x, y = y, color = g, linetype = g), linewidth = 0.8, na.rm = TRUE) +
        geom_line(data = data.frame(x = NA_real_, y = NA_real_, g = sprintf("Midpoint = %.2f", midpoint)),
                  aes(x = x, y = y, color = g, linetype = g), linewidth = 1.2, na.rm = TRUE) +
        geom_line(data = data.frame(x = NA_real_, y = NA_real_, g = sprintf("CI %d%%", cl_use * 100)),
                  aes(x = x, y = y, color = g, linetype = g), linewidth = 0.9, na.rm = TRUE) +
        scale_color_manual(
          name = NULL,
          values = stats::setNames(
            c(col.lower.main, col.lower.main, "black", col.lower.light),
            c(sprintf("LRL LabRI = %.2f", LRL),
...
###### 3.3.4.5.3. PANEL 3: Distribuição URL
    ...
    if (!is.null(sim.URL)) {
      ...
      p3 <- ggplot(df_url, aes(x = x)) +
        ...
        geom_vline(xintercept = URL, color = col.upper.main,
                   linewidth = 1.3, linetype = "solid") +
        geom_vline(xintercept = med_url, color = col.upper.main,
                   linewidth = 0.8, linetype = "dotted") +
        geom_vline(xintercept = row_ci$upper.lim.low, color = col.upper.light,
                   linewidth = 0.9, linetype = "dashed") +
        geom_vline(xintercept = row_ci$upper.lim.upp, color = col.upper.light,
                   linewidth = 0.9, linetype = "dashed") +
        scale_x_continuous(breaks = waiver(), n.breaks = 8, expand = waiver()) +
        labs(title = paste(sprintf("URL = %.2f", URL),
                           "|", n.sim, "Bootstrap replications"),
             subtitle = sprintf("CI %d%%: %.4f to %.4f",
                                cl_use * 100,
                                row_ci$upper.lim.low, row_ci$upper.lim.upp),
             x = xlab, y = "Absolute Frequency") +
        theme_mc +
        theme(
          plot.title = element_text(face = "bold", size = 9.5, hjust = 0.5),
      ...
      p3 <- p3 +
        geom_line(data = data.frame(x = NA_real_, y = NA_real_, g = sprintf("URL LabRI = %.2f", URL)),
                  aes(x = x, y = y, color = g, linetype = g), linewidth = 1.3, na.rm = TRUE) +
        geom_line(data = data.frame(x = NA_real_, y = NA_real_, g = sprintf("Median = %.2f", med_url)),
                  aes(x = x, y = y, color = g, linetype = g), linewidth = 0.8, na.rm = TRUE) +
        geom_line(data = data.frame(x = NA_real_, y = NA_real_, g = sprintf("Midpoint = %.2f", midpoint)),
                  aes(x = x, y = y, color = g, linetype = g), linewidth = 1.2, na.rm = TRUE) +
        geom_line(data = data.frame(x = NA_real_, y = NA_real_, g = sprintf("CI %d%%", cl_use * 100)),
...
n.truncado.clusters.2 <- ifelse(n.truncado.clusters > 50000,
                                50000,n.truncado.clusters)
...
jpeg("3_Outputs/Figures/Fig.23.jpeg", 
     width = 8 * 600, height = 8 * 600, units = "px", 
     res = 600, quality = 100)

CI_RI_LabRI$plot("Confidence intervals for LabRI method via parametric bootstrap using Monte Carlo simulation")
...
UL.IC.URL.LabRI<-round(CI_RI_LabRI$confidence.intervals$upper.lim.upp,
                         digits = casa.decimal2)

LL.IC.URL.LabRI<-round(CI_RI_LabRI$confidence.intervals$upper.lim.low,
                           digits = casa.decimal2)
...
UL.IC.LRL.LabRI<-round(CI_RI_LabRI$confidence.intervals$lower.lim.upp,
                           digits = casa.decimal2)

LL.IC.LRL.LabRI<-round(CI_RI_LabRI$confidence.intervals$lower.lim.low,
                           digits = casa.decimal2)
...
Fig.22 <- hist.mix.gauss.4 + 
  ...
  geom_vline(xintercept = LI.IR.LabRI.Fig.5.1,
             linetype = "solid", color = "#E57373", size = 0.6) +
  ...
  annotate("text", 
           x = LI.IR.LabRI.Fig.5.1, y = Inf, 
           label = paste0("LRL\n", format(LI.IR.LabRI.Fig.5.1, scientific = FALSE)),
           vjust = -0.3, hjust = 0.5, size = 3.2, fontface = "bold", color = "#E57373") +
  annotate("text", 
           x = LS.IR.LabRI.Fig.5.1, y = Inf, 
           label = paste0("URL\n", format(LS.IR.LabRI.Fig.5.1, scientific = FALSE)),
           vjust = -0.3, hjust = 0.5, size = 3.2, fontface = "bold", color = "#4682B4") +
  annotate("text", 
           x = Midpoint_IR, y = Inf, 
           label = paste0("Midpoint\n", format(Midpoint_IR, scientific = FALSE)),
           vjust = -0.2, hjust = 0.5, size = 3.2, fontface = "bold", color = "#404040") +
...
jpeg(filename = "3_Outputs/Figures/Fig.22.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(tmp, width = 4.4 * 600, height = 4.4 * 600, units = "px", 
     res = 300, quality = 100)

CI_RI_LabRI$plot("Fig. 23. Confidence intervals for LabRI method via parametric bootstrap using Monte Carlo simulation")

invisible(dev.off())
htmltools::img(src = knitr::image_uri(tmp), 
...
Fig.24 <- hist.mix.gauss.4 + 
  ...
  geom_segment(aes(x = Midpoint_IR, 
                   xend = Midpoint_IR,
                   y = 0, 
  ...
  geom_vline(xintercept = ifelse(is.na(URL.Comparativo) == TRUE, 
                                 LRL.Comparativo,
                                 URL.Comparativo), 
             colour = "#C376E0", 
             size = 0.6, 
  ...
  annotate("text", 
           ...
           y = Inf, 
           label = paste0("LRL\n", format(ifelse(is.na(LRL.Comparativo) == TRUE,
                                                 URL.Comparativo, LRL.Comparativo), 
                                          scientific = FALSE)),
           vjust = -0.3, hjust = 0.5, size = 3.2, fontface = "bold", color = "#C376E0") +
  
  annotate("text", 
           x = ifelse(is.na(URL.Comparativo) == TRUE, LRL.Comparativo, URL.Comparativo), 
           y = Inf, 
           label = paste0("URL\n", format(ifelse(is.na(URL.Comparativo) == TRUE,
...
Fig.24.new<-hist.mix.gauss.4 + 
  labs(x=mensurando,y="Absolute Frequency") +
  geom_vline(xintercept=ifelse(is.na(LRL.Comparativo)==TRUE,
  URL.Comparativo,LRL.Comparativo),linetype="solid",color="#C376E0",size=0.6) +
  geom_vline(xintercept=ifelse(is.na(URL.Comparativo)==TRUE,LRL.Comparativo,
  URL.Comparativo),colour="#C376E0",size =0.6,linetype="solid")

jpeg(filename = "3_Outputs/Figures/Fig.24.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
lado.Table <- ifelse(lado == "Two_sided", "Two sided",
              ifelse(lado == "Right_sided", "Right sided",
...
Table.19 <- kbl(Table.19.data.frame,
  ...
  row_spec(3,
           color = "black",
           background = ifelse(is.na(Bias.LS), "white",
...
Table.20 <- kbl(Table.20.data.frame,
  ...
  row_spec(3,
           color = "black",
           background = ifelse(is.na(Bias.LI), "white",
...
Haeckel_approach2 <- function(LI_IR, LS_IR, variable = "all", decimals = 2) {
  ...
  CV_E <- sqrt(exp(((log(LS_IR) - log(ifelse(is.na(LI_IR) || LI_IR == 0,
                                             ifelse(is.na(LS_IR), NA, 0.15 * LS_IR), LI_IR))) / 3.92)^2) - 1)
  ...
  pS_A.LI <- (ifelse(LI_IR == "", ifelse(LS_IR == "", NA, 0.15 * LS_IR), LI_IR) * Slope) + (0.2 * pS_A.Med)
  pS_A.LS <- (LS_IR * Slope) + (0.2 * pS_A.Med)
...
plot_UL_IE2 <- ggplot() +
  ...
  scale_color_manual(name = "Legend",
                     values = setNames("blue2", legend.label)) +
  labs(x = "Difference Between URLs (or Effect Size<sub>URLs</sub>)",
       y = "") +
  scale_x_continuous(breaks = pretty_breaks(n = 9)) +
  guides(fill = guide_legend(order = 1),
         color = guide_legend(order = 2,
...
Fig.25 <- if (isTRUE(lado == "Right_sided")) {
  ...
  ggarrange(plot_UL_IE, plot_LL_IE, heights = c(8, 8), align = "hv", ncol = 1, nrow = 2)
}

Fig.25

Fig.25.new <- if (isTRUE(lado == "Right_sided")) {
  plot_UL_IE2
} else {
  ggarrange(plot_UL_IE2, plot_LL_IE, heights = c(8, 8), align = "hv", ncol = 1, nrow = 2)
}

jpeg(filename = "3_Outputs/Figures/Fig.25.jpeg",
     width = 6 * 600, height = 4 * 600,
     units = "px", res = 600, quality = 100)
...
Fig.26 <- hist.mix.gauss.4 +
  ...
  geom_vline(aes(xintercept = ifelse(is.na(URL.RI.LabRI), LRL.RI.LabRI, URL.RI.LabRI),
                 colour = "URL of the\nLabRI Method."),
             size = 0.5, linetype = "solid") +
  geom_vline(aes(xintercept = ifelse(is.na(LRL.RI.LabRI), URL.RI.LabRI, LRL.RI.LabRI),
                 colour = "LRL of the\nLabRI Method."),
             size = 0.5, linetype = "solid") +
  geom_vline(aes(xintercept = ifelse(is.na(URL.Comparativo), LRL.Comparativo, URL.Comparativo),
                 colour = "URL of the\nReference Comparative."),
             size = 0.5, linetype = "solid") +
  geom_vline(aes(xintercept = ifelse(is.na(LRL.Comparativo), URL.Comparativo, LRL.Comparativo),
                 colour = "LRL of the\nReference Comparative."),
             size = 0.5, linetype = "solid") +
...
jpeg(filename = "3_Outputs/Figures/Fig.26.jpeg",
     width = 6 * 600, height = 4 * 600,
     units = "px", res = 600, quality = 100)
...
equivalence.test.LRL <- if (isTRUE(lado == "Right_sided")) {
  ...
  interpret.equivalence.test.LRL(lower_ci = LL.IC.LRL,
...
Table.23 <- kbl(Table.23.data.frame,
  ...
  row_spec(1,
           color = "black",
           background = ifelse(equivalence.test.URL == Equivalence.URL.txt, "palegreen",
                               ifelse(equivalence.test.URL == Probable.URL.txt |
                                        equivalence.test.URL == Possible.URL.txt |
                                        equivalence.test.URL == disabling.analyze.level.2, "#fdfd96",
                                      ifelse(equivalence.test.URL == No.Equivalence.URL.txt, "lightpink", "white")))) |>
  row_spec(2,
           color = "black",
           background = ifelse(equivalence.test.LRL == Equivalence.LRL.txt, "palegreen",
...
# Export full dataset to xlsx (unchanged)
write_xlsx(tabela.Quali,
           "3_Outputs/Spreadsheets/4_Table.24.xlsx")

# --- OPTIMIZATION: cap at 20,000 rows for HTML display -----------------------
...
Table.24 <- datatable(tabela.Quali.html,
                      class = "cell-border stripe",
...
<div style='text-align: justify; color: black;'>
  ...
  <a href='https://doi.org/10.1080/00365513.2024.2437613' target='_blank'>[66]</a>
</div>")
...
Titulo.Fig.27<-paste("Fig. 27. Box plot of the comparison of standardized (",
                     lab.Z,") results.")

Fig.27 <- ggplot(dataset.zlog_long, 
...
Fig.27.new <- ggplot(dataset.zlog_long, 
                        aes(x = factor(variavel),y = valor,fill = variavel)) +
  stat_boxplot(geom ='errorbar', linetype = 1, width = 0.2,size = 1) +
  geom_boxplot(outlier.shape = NA, lwd = 1) + 
  geom_hline(yintercept = c(-1.96, 1.96), 
...
jpeg(filename = "3_Outputs/Figures/Fig.27.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
<div style='text-align: justify; color: black;'>
  ...
  <a href='https://doi.org/10.1111/cea.14016' target='_blank'>[64]</a>,
  <a href='https://doi.org/10.4067/S0034-98872021000100013' target='_blank'>[65]</a>,
  <a href='https://doi.org/10.1007/978-1-4614-0562-7' target='_blank'>[67]</a>,
  <a href='https://doi.org/10.32614/CRAN.package.epiR' target='_blank'>[68]</a>
</div>")
...
jpeg(filename = "3_Outputs/Figures/Fig.28.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
<div style='text-align: justify; color: black;'><strong>References:</strong>
  ...
  <a href='https://www.ncbi.nlm.nih.gov/pmc/articles/PMC5111244/' target='_blank'>[69]</a>,
  <a href='https://doi.org/10.1373/clinchem.2015.243055' target='_blank'>[70]</a>,
  <a href='https://www.ncbi.nlm.nih.gov/pmc/articles/PMC4310060/' target='_blank'>[71]</a>,
  <a href='https://doi.org/10.1093/clinchem/45.3.321' target='_blank'>[72]</a>,
  <a href='https://doi.org/10.1515/CCLM.2004.123' target='_blank'>[73]</a>,
  <a href='https://doi.org/10.1373/clinchem.2003.027953' target='_blank'>[74]</a>,
  <a href='https://doi.org/10.4097/kjae.2016.69.6.555' target='_blank'>[75]</a>,
  <a href='https://doi.org/10.1515/cclm-2016-0307' target='_blank'>[76]</a>,
  <a href='https://doi.org/10.1016/S0009-8981(03)00133-5' target='_blank'>[77]</a>,
  <a href='https://doi.org/10.3109/00365518809088757' target='_blank'>[78]</a>
</div>"
...
equivalence.test.interp.URL <- ifelse(is.na(equivalence.test.interp.URL),
                                      NA.URL2.txt,
...
equivalence.test.interp.LRL <- ifelse(is.na(equivalence.test.interp.LRL),
                                      NA.LRL2.txt,
...
ifelse(equivalence.test.interp.URL==No.Equivalence.URL2.txt|
         equivalence.test.interp.URL==No.Possible.URL2.txt,
                                            ...
                                            ifelse(equivalence.test.interp.URL==disabling.analyze.level.2,
                                                   disabling.analyze.level.2,"N/A"))))) |>
  row_spec(2, color = "black", background = ifelse(conclusion.flagging.rates.LabRI==txt.conclusion.LabRI.ok,
                                                   "palegreen","#fdfd96")) |> 
  row_spec(3, color = "black", background = ifelse(conclusion.flagging.rates.RC==txt.conclusion.RC.ok,
                                                   "palegreen","#fdfd96")) |>
  column_spec(1, bold = TRUE, color = "White", background = "#1976d2")
