...
Packages <- c(
  "AID", "DT", "FactoMineR", "KernSmooth", "MASS", "MethComp","RVAideMemoire",
  "callr","calibrate", "cartography","cluster","data.table","datawizard",
  "devtools","digest", "dplyr","epiR", "factoextra", "ffp", "forecast", 
  "ggplot2", "ggpubr","ggtext", "grid", "gt", "imputeTS","installr","irr",
  "janitor", "kableExtra", "knitr", "lattice", "lubridate","mclust", "mixR",
  "modeest", "moments", "multimode","multiway", "nortest","openxlsx",
  "patchwork","pandoc","plotly", "prettydoc", "qqplotr", "readr", "readxl",
  "refineR", "reflimR", "rmarkdown", "scales", "shiny","shinyjs",
  "shinythemes", "stats", "stringi","systemfonts", "tidyr", "tools","univOutl",
  "utf8","xfun", "writexl","zlog"
)
...
##### 1.3.2.1. Read the first lines to infer the decimal mark
...
Titulo.Table.2<-htmltools::tags$caption(
                       ...
                       htmltools::HTML("<b>Table 2.</b> Individual results in the analyzed dataset."))
...
###### 3.1.1.2.3. xlim "smart" (label-aware) ###################################
...
###### 3.1.1.2.9. If the text still touches the panel ##########################
...
################################################################################
##### 3.1.1.4. Fallback RI selection function (NP, refineR, and reflimR) #######
################################################################################
...
###### 3.1.1.4.2. Decision rule (NP preponderant) ##############################
...
###### 3.1.1.5.1. Title wrapping + extra top margin ############################
...
###### 3.1.1.5.3. Draw ONLY axis lines (no frame) ##############################
...
###### 3.1.1.5.4. Draw wrapped title (left-aligned), preventing clipping #######
...
###### 3.1.1.5.5. Reference limits (VERTICAL lines on x-axis) + labels #########
...
####### 3.1.1.5.5.1. Place labels OUTSIDE the plotting region, close to the top of the panel
...
################################################################################
##### 3.1.1.11. Reference limits NP LabRI low-count (Before) ###################
################################################################################
...
################################################################################
##### 3.1.1.12. Reference Interval refineR (before) ############################
################################################################################
...
################################################################################
##### 3.1.1.13. Decision: Histogram from refineR or LabRI NP low-count (before)# 
################################################################################
...
################################################################################
##### 3.1.1.14. Reference Interval reflimR (before) ############################
################################################################################


###### 3.1.1.14.1. If the side is Right_sided ##################################
...
###### 3.1.1.14.2. Statistical parameters (mean and SD) reflimR ################
...
###### 3.1.1.14.3. Reference limits reflimR ####################################
...
###### 3.1.1.14.5. If the side is Two_sided ####################################    
...
###### 3.1.1.14.6. Statistical parameters (mean and SD) reflimR ################
...
###### 3.1.1.14.7. Reference limits reflimR ####################################
...
################################################################################
##### 3.1.1.15. Fallback RI selection among NP, refineR, and reflimR (before) ##
################################################################################
...
################################################################################
##### 3.1.2.8. Decision: Histogram from refineR or LabRI NP low-count (after) ##
################################################################################
...
###### 3.1.2.11.5. If the side is Two_sided ####################################    
...
###### 3.1.2.11.6. Statistical parameters (mean and SD) reflimR ################
...
###### 3.1.2.11.7. Reference limits reflimR ####################################
...
##### 3.2.10.1. Mixfit Cluster Audit function ##################################
...
###### 3.2.10.1.1. Identify the dominant component
  
  i_dom <- which.max(ifelse(valid_mu, pi, -Inf))
  
###### 3.2.10.1.2. Internal calculation functions
  
####### 3.2.10.1.2.1. Bhattacharyya Coefficient
...
######## 3.2.10.1.2.1.1. Bhattacharyya distance
   
    DB <- 0.25 * log(((s1^2 + s2^2) / 2) / (s1 * s2)) +
          ((m1 - m2)^2) / (4 * (s1^2 + s2^2))
    
######## 3.2.10.1.2.1.2. Convert to coefficient
 
    exp(-DB)
  }
  
####### 3.2.10.1.2.2. Ashman's D
...
####### 3.2.10.1.2.3. Adjacency Analysis
...
######## 3.2.10.1.2.3.1. Skip if: dominant, invalid, or not adjacent
...
######## 3.2.10.1.2.3.2. Calculate metrics for adjacent components
...
######## 3.2.10.1.2.3.4. Unification Decision Logic (Hierarchical)

######### 3.2.10.1.2.3.4.1. PRIORITY 1
    
    if (!is.na(similar_weight) && similar_weight) {
      unify <- TRUE
      
######### 3.2.10.1.2.3.4.2. PRIORITY 2 
      
    } else if (!is.na(poorly_separated) && poorly_separated) {
      unify <- TRUE
      
######### 3.2.10.1.2.3.4.3. PRIORITY 3
...
###### 3.2.10.1.3. Definition of the Unified Group and Truncation Limits
...
####### 3.2.10.1.3.1. Truncation limits
...
###### 3.2.10.1.4. Individual Limits and Summary
...
####### 3.2.22.2.1.1. Check if num_clusters > 1
...
####### 3.2.22.2.1.2. Convert ratio to percentage if needed
  
  if (sum(proportions) != 100) {
    proportions <- proportions / sum(proportions) * 100
  }
  
####### 3.2.22.2.1.3. Check if any cluster has proportion > 50%
...
####### 3.2.22.2.1.4. Calculate distance between cluster means and global midpoint
  
  distances <- abs(midpoints[main_clusters] - global_midpoint)
  
####### 3.2.22.2.1.5. Find the cluster with the smallest distance
  
  min_distance_cluster <- main_clusters[which.min(distances)]
  
####### 3.2.22.2.1.6. Check distance of other clusters relative to main cluster
...
####### 3.2.22.2.1.7. Calculate the lower and upper reference limits
...
####### 3.2.22.2.1.8. Apply InvBoxCox if log-transformed
...
####### 3.2.22.2.1.10. Vector clusters function


vector.clusters <- function(k.new,vetor.Mi,vetor.Sigma,vetor.Weights) {
  
####### 3.2.22.2.1.10.1. Checks if k.new is within the allowed range
  
  if (k.new < 1 || k.new > 7) {
    stop("k.new must be a value between 1 and 7")
  }
  
####### 3.2.22.2.1.10.2. Update the vectors according to the specified n clusters
...
####### 3.2.22.2.1.10.3. Returns the fitted vectors in a list
...
######## 3.2.22.2.2.3.1. Combine all values into a single vector
  
  all_values <- c(outside1, outside2)
  
######## 3.2.22.2.2.3.2. Check if any value is greater than 0.06
...
###### 3.2.22.3.2. Calculating intermediate and final variables
...
###### 3.2.22.3.3. Creating a data frame with all variables

  results_table <- data.frame(CV_E, pCV_A, Med, pS_A.Med, 
                              Slope, Intercept,
                              pS_A.LI, pS_A.LS, pD_LI, pD_LS)
  
###### 3.2.22.3.4. Returning the specified variable or all variables
...
###### 3.2.22.5.1. Hartigan-Wong Centroid Reference Limits Algorithm
...
###### 3.2.22.5.2. Defining the limits
...
###### 3.2.22.5.3. K-means clustering to find the representative thresholds
  
  kmeans_result<-kmeans(limites[,c("Limite_inferior","Limite_superior")],
                        algorithm = "Hartigan-Wong",centers = 1)

###### 3.2.22.5.4. More representative limits
...
###### 3.2.22.5.5. Adding K-means results to the data frame
  
  limites$Cluster <- factor(kmeans_result$cluster)
  
###### 3.2.22.5.6. Defining axis limits
...
###### 3.2.22.5.9. Returning the results
...
###### 3.2.22.5.10. Defining the limits
...
###### 3.2.22.5.11. K-means clustering to find the representative thresholds
  
  kmeans_result<-kmeans(limites[,c("Limite_inferior",
                                   "Limite_superior")],
                        centers = 1)

###### 3.2.22.5.12. More representative limits
  
  limite_inferior_escolhido <- kmeans_result$centers[1, "Limite_inferior"]
  limite_superior_escolhido <- kmeans_result$centers[1, "Limite_superior"]
  
###### 3.2.22.5.13. Adding K-means results to the data frame
  
  limites$Cluster <- factor(kmeans_result$cluster)
  
###### 3.2.22.5.14. Defining axis limits
  
  x_limits <- range(limites$Limite_inferior, limite_inferior_escolhido)
  y_limits <- range(limites$Limite_superior, limite_superior_escolhido)
  
##### 3.2.22.5.15. Creating the graph
...
##### 3.2.22.5.16. Plot the graph
  
p1<-ggplotly(p1)

print(p1)

##### 3.2.25.5.17. Returning the results
...
##### 3.2.23.6. Step 6: Table 15 and Fig. RIs before and #######################
...
jpeg(filename = "3_Outputs/Figures/Fig.21.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
n.bootstrap<-ifelse(n.truncado.clusters<1000,20000,10000)
...
###### 3.3.4.4.1. STEP 1 - Generate simulations (memory-optimized)

  sim.matrix <- matrix(rnorm(n.sim * n, mean = mu.t, sd = sigma.t),
                       nrow = n.sim, ncol = n)
  sim.matrix <- box_cox_inv(sim.matrix, lambda)

###### 3.3.4.4.2. STEP 2 - Percentiles of each simulation (distribution of limits)

  if (lado == "Two_sided") {
    sim.LRL <- apply(sim.matrix, 1, quantile, probs = probs.LRL)
    sim.URL <- apply(sim.matrix, 1, quantile, probs = probs.URL)
  } else if (lado == "Right_sided") {
    sim.LRL <- NULL
    sim.URL <- apply(sim.matrix, 1, quantile, probs = probs.URL)
  } else if (lado == "one_sided_lower") {
...
###### 3.3.4.4.2.1. Random thinning for visualization (memory-safe)

  n.total  <- length(sim.matrix)
  n.sample <- min(n.total, max.combined)
  combined <- sim.matrix[sample.int(n.total, n.sample)]
  combined <- combined[is.finite(combined)]

  rm(sim.matrix)
  gc()

###### 3.3.4.4.3. STEP 3 - Calculate the CI for each requested confidence level

  ci_results <- data.frame(
    conf.level     = numeric(),
    lower.lim.low  = numeric(),
    lower.lim.upp  = numeric(),
    upper.lim.low  = numeric(),
...
####### 3.3.4.4.4.1. CI data (use the first conf.level)

    cl_use <- conf.level[1]
    row_ci <- ci_results[ci_results$conf.level == cl_use, ]

####### 3.3.4.4.4.2. Standardized colors

    col.lower.main   <- "#E53935"
    col.lower.light  <- "#E57373"
    col.lower.fill   <- adjustcolor("#E53935", alpha.f = 0.15)
    col.upper.main   <- "#1976D2"
    col.upper.light  <- "#64B5F6"
    col.upper.fill   <- adjustcolor("#1976D2", alpha.f = 0.15)

####### 3.3.4.4.4.3. General title
...
###### 3.3.4.5.1. PANEL 1: Histogram of combined data
  ...
  layout(
    ...
    annotations = list(
      list(
        ...
        y = 1.02,
        yref = "paper",
        text = paste0("<b>LRL</b><br>","<b>", 
                      format(LI.IR.LabRI.Fig.5.1,scientific = FALSE),"</b>"),
        showarrow = FALSE,
        font = list(size = 9, color = "#E57373"),
        xanchor = "center",
        yanchor = "bottom"),
      list(
        ...
        y = 1.02,
        yref = "paper",
        text = paste0("<b>URL</b><br>","<b>", 
                      format(LS.IR.LabRI.Fig.5.1,scientific = FALSE),"</b>"),
        showarrow = FALSE,
        ...
        y = 1.01, 
        yref = "paper",
        text = paste0("<b>Midpoint</b><br>","<b>", 
                      format(Midpoint_IR,scientific = FALSE),"</b>"),
        showarrow = FALSE,
