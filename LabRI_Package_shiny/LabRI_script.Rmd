...
################################################################################
#### 1.2. Pareto "onboard" (sem ggQC) — drop-in para ggQC::stat_pareto #########
########## Requer: library(ggplot2) ############################################
################################################################################


# ggproto da stat (ordena por y decrescente, soma por categoria, acumula se cumsums=TRUE)

StatPareto <- ggplot2::ggproto(
  "StatPareto", ggplot2::Stat,
  required_aes = c("x", "y"),
  compute_panel = function(data, scales, cumsums = TRUE, na.rm = FALSE) {

    # remove NAs de x/y
    data <- data[!is.na(data$x) & !is.na(data$y), , drop = FALSE]
    if (nrow(data) == 0L) {
      return(data.frame(x = numeric(), y = numeric(), group = integer()))
    }

    # agrega (caso a mesma categoria apareça mais de uma vez)
    agg <- stats::aggregate(y ~ x, data = data, FUN = sum, na.rm = na.rm)

    # ordena por y decrescente e fixa níveis de x nessa ordem
    ord <- order(agg$y, decreasing = TRUE, na.last = NA)
    agg <- agg[ord, , drop = FALSE]
    agg$x <- factor(agg$x, levels = agg$x, ordered = TRUE)

    # acumulado (para linha/ponto)
    if (isTRUE(cumsums)) {
      agg$y <- cumsum(agg$y)
    }
...
# Wrapper compatível com ggQC::stat_pareto

stat_pareto <- function(mapping = NULL, data = NULL,
                        geom = "point", position = "identity",
                        show.legend = NA, inherit.aes = TRUE, group = 1,
                        na.rm = FALSE,
                        point.color = "black", point.size = 2,
  ...
  Points <- ggplot2::layer(
    stat = StatPareto, data = data,
    mapping = ggplot2::aes(group = 1),
    geom = geom, position = position, show.legend = show.legend,
    inherit.aes = inherit.aes,
    params = list(na.rm = na.rm, cumsums = TRUE,
  ...
  # Barras (valor bruto). Se passar uma cor única, usa-a; se passar vetor, usa a primeira.
  
  fill_val <- if (length(bars.fill) >= 1) bars.fill[1] else "grey70"
  Bars <- ggplot2::layer(
    stat = StatPareto, data = data,
    mapping = ggplot2::aes(group = 1),
    geom = "col", position = position, show.legend = show.legend,
    inherit.aes = inherit.aes,
    params = list(na.rm = na.rm, cumsums = FALSE,
                  color = "black", fill = fill_val, ...)
  )

  list(Bars, Line, Points, SEC.scaleY)
}


################################################################################
#### 1.3. Reloading packages ###################################################
################################################################################
...
################################################################################
#### 1.4. Reading the file - Start Data cleaning ###############################
################################################################################
...
read_dataset_column <- function(file_name, column_name) {
  ...
  # Use the file name directly (it will now be the full path passed by Shiny)
  file_path <- file_name
...
# Adjusted to work with the full file path and column name from Shiny
resultados <- data.frame(read_dataset_column(File_Name, Column_Name))

# Clean and process the results
...
Sk_PruneTails <- function(dataset) {
  ...
  if (!is.na(lower_limit) & !is.na(upper_limit)) {
    processed_dataset <- dataset[dataset >= lower_limit & dataset <= upper_limit]} else {
...
ds_orig_filtered.sk<-dataset_PruneTails$processed_dataset
  ...
  ds_transf_filtered.sk<-forecast::BoxCox(ds_orig_filtered.sk,lambda.box.cox.sk)
    ...
    sk_orig <- bowley(ds_orig_filtered.sk[!is.na(ds_orig_filtered.sk)], 
                      alpha = 0.25)
 
    sk_transf <- bowley(ds_transf_filtered[!is.na(ds_transf_filtered)], 
                        alpha = 0.25)
...
jpeg(filename = "3_exports/Fig.10._RI_refLimR_after_exclusion_outliers.jpeg", width = 6 * 600, height = 4 * 600, units = "px", 
     res = 600, quality = 100)
...
titulo.fig.14<-"Fig. 14. BIC plot indicating the number of selected clusters."
...
fig.14<-ggplot(data=dados.BIC2, 
        ...
        geom_line(size=1.5) +
        labs(title=titulo.fig.14,x="Number of Components (Clusters)",
...
jpeg(filename = "3_exports/Fig.14._BIC_plot.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_exports/Fig.15._Density_plot_modes_Antimodes.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
Titulo.fig.15<-paste("Fig. 15. Density Plot with Modes and Anti-modes.")

Fig.15 <- plot(multimoda,addLegend=TRUE,xlab=mensurando,ylab="Density",
          main=Titulo.fig.15)
...
titulo.fig.16<-paste("Fig. 16. Histogram with",n.misturas,
...
jpeg(filename = "3_exports/Fig.16._Histogram_Gaussian_Mix.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
titulo.fig.17<-paste("Fig. 17. Histogram and 'New Truncated Global Distribution':",
                      range.truncado.clusters)

Fig.17 <-hist.mix.gauss.2 + 
         labs(title=titulo.fig.17, x=mensurando, y = "Density") +
...
jpeg(filename = "3_exports/Fig.17._Histogram_New_Truncated.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
titulo.fig.18<-paste("Fig.18. Density plot and Box-Plot of the data from the 'New Truncated Global Distribution' (n<sub>New2</sub> =",n.truncado.clusters,")")
...
jpeg(filename = "3_exports/Fig.18._Density_plot_Box-Plot_New_Truncated.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
titulo.fig.19<-paste("Fig.19. Ogive Graph of the 'New Truncated Global Distribution' data  (n<sub>New2</sub> =",n.truncado.clusters,").")

Fig.19<-ggplot(dataset.truncado.clusters,
        aes(dataset.IR)) + 
        stat_ecdf(geom = "step", pad = FALSE,size =1.2) +
        labs(title=titulo.fig.19,
       x=mensurando,y="Cumulative frequency") +
  theme(plot.title=element_textbox_simple(size=11,face = "bold"))

Fig.19

Fig.19.new<-ggplot(dataset.truncado.clusters,
        aes(dataset.IR)) + 
        stat_ecdf(geom = "step", pad = FALSE,size =1.2) +
        labs(x=mensurando,y="Cumulative frequency") +
  theme(plot.title=element_textbox_simple(size=11,face = "bold"))
...
jpeg(filename = "3_exports/Fig.19._Ogive_Graph_New_Truncated.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
titulo.fig.20<-paste("Fig.20. Q-Q Plot of the 'New Truncated Global Distribution' data (n<sub>New2</sub>=",n.truncado.clusters,").")


Fig.20<-ggplot(dataset.truncado.clusters,
...
Fig.20.new<-ggplot(dataset.truncado.clusters,
                 mapping = aes(sample = dataset.IR)) +
  stat_qq_band(geom="qq_band",distribution="norm",qtype=7,bandType="pointwise",
               fill = "gold4",alpha = 0.4,conf = 0.95) +
  stat_qq_line(size=1,color="royalblue4") +
...
jpeg(filename = "3_exports/Fig.20._Q-Q_Plot_New_Truncated.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
cat('
<div style="text-align: justify; line-height: 1.8; color: #0033CC; font-size: 1.2em;">
<p><strong>3.2.4.1. Approach applied in a scenario with 1 cluster in the truncated distribution:</strong></p></div>
...
<br><br>
<div style="text-align: justify; line-height: 1.8; color: #0033CC; font-size: 1.2em;">
<p><strong>3.2.4.2. Approach applied in a scenario with multiple clusters in 
the truncated distribution:</strong></p>
</div>
...
jpeg(filename = "3_exports/Fig.21._RIK-means_Centroid.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
Half_Gaussian_CI<-function(mode,median,mean,lambda,logtransform,
                           LRL_LabRI,URL_LabRI,ic,digits){
  ...
  # Step A.2 - Defining Empirical Standard Deviation (SDe) and Standard Error (SEe)
   ...
   k_IC <- function(ic) {
     if (is.null(ic) || length(ic) == 0 || is.na(ic)) 
       {return(list(factor = 1.645, label  = "90% CI"))}
  
  # Input verification
     
  if (!is.numeric(ic) || ic < 0 || ic > 1) 
    {stop("The CI value must be numeric and between 0 and 1.")}
  
  # Factor selection and labeling
     
  if (ic == 0.90) {return(list(factor = 1.645, label = "90% CI"))} 
     else if (ic == 0.95) {return(list(factor = 1.96, label = "95% CI"))} 
     else if (ic == 0.99) {return(list(factor = 2.576, label = "99% CI"))} 
     else {stop("Invalid CI. Use only 0.90, 0.95, or 0.99.")}}

   factor.k.IC<-k_IC(ic)$factor
   
   factor.k.IC.label<-k_IC(ic)$label
   ...
   SE.empirical.URL<-(SD.empirical.URL/sqrt(n.truncado.clusters))
   SE.empirical.LRL<-(SD.empirical.LRL/sqrt(n.truncado.clusters))   
   
   SE.fractile.URL<-sqrt((2+factor.k.IR^2)/2)*SE.empirical.URL
   SE.fractile.LRL<-sqrt((2+factor.k.IR^2)/2)*SE.empirical.LRL
   
   margin_error.URL<-factor.k.IC*SE.fractile.URL
   margin_error.LRL<-factor.k.IC*SE.fractile.LRL
   ...
   UL.IC90.LRL.LabRI<-LRL_LabRI+margin_error.LRL
   ...
   LL.IC90.LRL.LabRI<-LRL_LabRI-margin_error.LRL
   ...
   UL.IC90.URL.LabRI<-URL_LabRI+margin_error.URL
   ...
   LL.IC90.URL.LabRI<-URL_LabRI-margin_error.URL
   ...
   result <- list(Midpoint = Midpoint_finale,
                  ...
                  Factor_k_IC_label = factor.k.IC.label)
...
IC_label<-CI.info$Factor_k_IC_label
...
jpeg(filename = "3_exports/Fig.22._RI_LabRI_Method.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_exports/Fig.23._RI_Comparative_Reference.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_exports/Fig.24._Equivalence_plots.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_exports/Fig.25._Histogram_with_RLs.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
Titulo.Table.24<-paste0(
  "<span style='display:block; text-align: justify; color: #000;'>",
  "<strong>Table 24.</strong> 3×3 Contingency table comparing value interpretations ",
...
# Cálculo do kappa de Fleiss (global e detalhado) e interpretação
...
resumo_grupos_Var1 <- function(teste) {
  # 1. categorias únicas, padronizadas
  ...
  # 3. junta tudo numa única string
...
# Inicialização dos nomes das colunas e estrutura da tabela de saída
...
# Monta a tabela de resultados conforme o parâmetro 'lado' selecionado
...
if (classe_select == "normal, high, low") {
  ...
  Kappa.Fleiss.Table_df <- data.frame(lista.nomes.kappa.Fleiss.Table,
    ...
    # Preenche resultados para Normal, High e Global
    ...
    # Preenche resultados só para Low, Normal e Global
  ...
  # Colore a célula de interpretação segundo a classificação
...
# Define o rodapé explicativo para interpretação dos valores do kappa
...
# Monta e formata a tabela HTML para saída em relatório
...
Titulo.Table.25 <- paste("<div style='text-align: justify; color: black;'><strong>Table 25.</strong> Fleiss kappa statistics for attribute agreement analysis between LabRI method and Comparative Reference in laboratory test Classifications.</div>")
  ...
  # Organiza todos os resultados em uma lista e retorna
...
calculate_zlog <- function(dataset,lado,
  ...
  # 1. Detecta automaticamente o nome da única coluna do dataset
  Column_Name <- names(dataset)[1]
  
  # 2. Define as probabilidades conforme o lado
  prob <- if (lado == "Right_sided") c(0.000001, 0.95) else c(0.025, 0.975)
  
  # 3. Calcula zlog com os dois intervalos
  zlog_IR_comparativo <- zlog::zlog(x = dataset[[Column_Name]],
                                    limits = IR_comparativo_dataset,
                                    probs  = prob)
  
  zlog_IR_LabRI <- zlog::zlog(x = dataset[[Column_Name]],
                              limits = IR_LabRI_dataset,
                              probs  = prob)
  
  # 4. Cria novos nomes de coluna dinamicamente
  col_ref1 <- "zlog_IR_LabRI"
  col_ref2 <- "zlog_IR_comparativo"
  
  # 5. Adiciona as colunas ao dataset original
...
Titulo.Fig.26<-"Fig. 26. Box-Plot of the comparison of standardized results based on the \nestimated reference interval by LabRI method and the defined Comparative Reference."
...
jpeg(filename = "3_exports/Fig.26._Box_Plot_standardized_results.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
...
jpeg(filename = "3_exports/Fig.27._Scatter_plotBox_standardized_results.jpeg", 
     width = 6 * 600, height = 4 * 600, 
     units = "px", res = 600, quality = 100)
